...
router.post('/add_assessment_type', function (req, res) {
    try {
        Question_Type.findOne({
            'question_type_name': {
                $regex: new RegExp(req.body.question_type_name, 'i')
            }
            ...
            if (questionType) {
                ...
                Versions.findById({
                    _id: req.body.version_id
                }, (err, version) => {
                    if (err) {
                        return res.json(500, err)
                    }
                    if (version) {
                        question_type.version_name = version.version_name;
...
router.get('/get_question_list/:question_section_id/:franchisor_id', function (req, res) {
    ...
    try {
        Question.find({
            question_section_id: req.params.question_section_id,
            franchisor_id: req.params.franchisor_id
        }, function (err, ques) {
            if (err) {
                return res.send({
        ...
        Question.findById({
            ...
            if (ques) {
                if (ques.question_EN == req.body.question_EN) {
                    ...
                    Question.find({
                        question_EN: {
                            $regex: new RegExp(req.body.question_EN, 'i')
                        }
                    }, function (err, q_name) {
                        if (err) {
                            return res.send({
...
router.delete('/delete_question_type/:question_type_id', function (req, res) {
    try {
        Question_Type.findByIdAndRemove({
